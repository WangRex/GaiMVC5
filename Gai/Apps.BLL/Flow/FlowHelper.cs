...
namespace Apps.BLL.Flow
{
    public class FlowHelper
    {
        ...
        public string GetCurrentStepCheckIdByStepCheckModelList(List<Flow_FormContentStepCheck> stepCheckModelList)
        {
            string stepCheckId = "";
            for (int i = stepCheckModelList.Count() - 1; i >= 0; i--)
            {
                //获得在进行中的单子
                if (stepCheckModelList[i].State == FlowStateEnum.Progress.ToString())// || stepCheckModelList[i].State == (int)FlowStateEnum.Reject
                {
                    stepCheckId = stepCheckModelList[i].Id.ToString();
                    if (i != 0)//查看上一个审核状态
                    {
