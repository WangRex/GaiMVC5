...
namespace Apps.Web.Core.Signalr
{
    ...
    public class WebIM : Hub
    {
        ...
        SysUserBLL m_BLL = new SysUserBLL();

        public static List<SysOnlineUser> list = new List<SysOnlineUser>();
        ...
        public override Task OnConnected()
        {
            if (list.Count()==0)
            {
                IQueryable<SysUser> queryData = m_BLL.m_Rep.FindList(a => a.State == "1");
                List<SysOnlineUser> modelList = (from r in queryData
                                                      select new SysOnlineUser
                                                      {
                                                          UserId = r.KEY_Id,
                                                          TrueName = r.TrueName,
        ...
        {
            //获取缓存中的用户列表
            SysOnlineUser model = new SysOnlineUser();
            //用户链接时调用
